...
const CAPTURE_SECS   = 4.0;  // ventana más larga para capturar más frames
const MIN_CONF       = 0.20; // umbral muy bajo - casi siempre acepta
const MIN_FRAMES_PRIMARY = 2; // REDUCIDO - solo necesita 2 frames para ser válido
const MIN_PERCENTAGE = 15;   // NUEVO - la emoción debe aparecer al menos 15% del tiempo
const IMG_SIZE       = 96;
const NEUTRAL_NAME   = "neutral";

// Emociones primarias que SIEMPRE tienen prioridad sobre neutral
const PRIMARY_EMOTIONS = ["angry", "fear", "happy", "sad", "surprise"];

// Candado temprano (solo para emociones primarias)
const LOCK_CONF      = 0.35; // confianza mínima por frame para lock
const LOCK_CONSEC    = 7;    // frames consecutivos para lock inmediato
const NEUTRAL_WEIGHT = 0.001; // neutral NO cuenta prácticamente NADA

/* emoción -> archivo de animación */
...
document.getElementById("cfgCapture").textContent = `${CAPTURE_SECS}s`;
document.getElementById("cfgConf").textContent    = MIN_CONF.toFixed(2);

/* ====== ESTADO ====== */
let session = null;
let labels  = [];
let phase   = "loading"; // loading | idle | countdown | capturing | showing
let countdown = COUNTDOWN_SECS;
let history = [];                      // [{t,label,conf}]
let consec  = { label: null, n: 0 };   // para lock
...
function setPhase(p){
  phase = p;
  if (p === "idle") {
    setPill("Listo","pill-idle");
    overlayMessage("Haz clic en Iniciar");
  } else if (p === "countdown"){
    setPill("Preparando","pill-warn");
  } else if (p === "capturing"){
    setPill("Detectando","pill-ok");
    overlayMessage("Capturando…");
  } else if (p === "showing"){
    setPill("Animación","pill-show");
    overlayMessage("");
  }
}
...
function overlayMessage(msg){ $overlay.textContent = msg || ""; }
...
async function populateCameraList(selectedId = null){
  // Importante: para ver nombres, primero hay que haber otorgado permiso de cámara
  const devices = await navigator.mediaDevices.enumerateDevices();
  const cams = devices.filter(d => d.kind === "videoinput");
...
$cameraSelect?.addEventListener("change", async (e) => {
  ...
  try {
    ...
    if (phase === "idle") overlayMessage("Cámara cambiada.");
  } catch (err) {
...
$btnRetry.addEventListener("click", async ()=>{
  // volver a idle y reabrir cámara
  $resBox.classList.add("hidden");
  $anim.classList.add("hidden");
  $video.classList.remove("hidden");
  $btnRetry.disabled = true;
  // reabrir la cámara actual (o guardada)
  const savedId = localStorage.getItem("emotion_cam_id");
...
async function captureWindow(){
  ...
  while ((performance.now() - start)/1000 < CAPTURE_SECS){
    const locked = await stepInference(); // true si hubo lock
    if (locked) return; // ya decidió y mostró animación
    await sleep(0);
  }
  decide(); // votación robusta
}
...
async function stepInference(){
  ...
  // Crop tipo "cover" al cuadrado IMG_SIZE
  ...
  // ---- Candado temprano (solo primarias) ----
...
function decide(){
  ...
  // Usa solo el 60% central (descarta 20% inicial y 20% final)
  const n = history.length;
  const i0 = Math.floor(n*0.20), i1 = Math.ceil(n*0.80);
  const core = history.slice(i0, i1);

  // Conteo ponderado (neutral pesa menos) y conteo de primarias
  const votes = new Map(); // label -> {wcount, sum, count}
  let primaryFrames = 0;
  for (const h of core){
  ...
  // Preferencia por no-neutral; si todo es neutral, toma el primero
  let bestLabel = null;
...
function showDecision(label, conf){
  // Decisión final: apaga cámara y muestra animación
  stopTracks();
  setPhase("showing");
